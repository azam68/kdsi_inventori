...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($lokasi)) {
        ...
        if ($upload) {
            ...
            $stmt = $koneksi->prepare("UPDATE users SET nik=?, nama=?, telepon=?, username=?, password=?, level=?, foto=? WHERE id=?");
            $stmt->bind_param("sssssssi", $nik, $nama, $telepon, $username, $hashedPassword, $level, $foto, $id);
            $stmt->execute();
            ...
            if ($stmt->affected_rows > 0) {
                ...
                echo '<center><div class="alert alert-danger">Upss...!!! Terjadi kesalahan saat mengubah data.</div></center>';
            }
        ...
        $stmt = $koneksi->prepare("UPDATE users SET nik=?, nama=?, telepon=?, username=?, password=?, level=? WHERE id=?");
        $stmt->bind_param("ssssssi", $nik, $nama, $telepon, $username, $hashedPassword, $level, $id);
        $stmt->execute();
